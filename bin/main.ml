...
and withdraw_ db u = 
  ...
  match read_line () with
  |exception End_of_file -> ()
  |inp -> if "brb" = (String.lowercase_ascii inp) then withdraw__ db u (String.lowercase_ascii inp)
  else if "usd" = (String.lowercase_ascii inp) then withdraw__ db u (String.lowercase_ascii inp)
  else ANSITerminal.(print_string [blue; Bold] ("\nInvalid input. \n"));
...
and main () = 
  ...
  | Database x -> (Yojson.Basic.from_file (data_dir_prefix ^ (List.hd x) ^ ".json") |> Yojson.Basic.to_string |> from_json |> login)
  | _ -> main ()
end
